...
describe('Testing util methods', function () {
    ...
    it("Generated visited strings should match expected string",()=>{
        expect(util.genUniqueVisitedString({url:"http://localhost",method:"get"})).to.be.equal("[GET] http://localhost/")
        expect(util.genUniqueVisitedString({url:"http://localhost",method:"GET"})).to.be.equal("[GET] http://localhost/")
        expect(util.genUniqueVisitedString({url:"http://localhost",method:"POST"})).to.be.equal("[POST] http://localhost/")
        expect(util.genUniqueVisitedString({url:"http://localhost/dashboard.html",method:"post"})).to.be.equal("[POST] http://localhost/dashboard.html")
        expect(util.genUniqueVisitedString({url:"http://localhost?h=cool",method:"HEAD"})).to.be.equal("[HEAD] http://localhost/?h=cool")
    })
